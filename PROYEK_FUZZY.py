...
# FUNGSI CRUD DATASET
# ─────────────────────────────────────────────────────────────
CSV_PATH = "data_baru.csv"

NUMERIC_COLS = ["cleanliness", "value", "location", "rooms", "sleep_quality", "overall"]
ALL_COLS     = ["hotel_id", "cleanliness", "value", "location", "rooms", "sleep_quality", "overall"]

def load_dataset_crud():
    """Load dataset langsung dari CSV (tanpa cache) untuk keperluan CRUD."""
    try:
        _df = pd.read_csv(CSV_PATH, on_bad_lines="skip", low_memory=False)
        for c in NUMERIC_COLS:
            if c in _df.columns:
                _df[c] = pd.to_numeric(_df[c], errors="coerce")
        _df = _df.dropna(subset=["cleanliness", "value", "location", "rooms", "sleep_quality", "overall"]).copy()
        _df = _df.reset_index(drop=True)
        _df["hotel_id"] = _df["hotel_id"].astype(str).str.split(".").str[0]
        return _df
    except Exception as e:
        st.error(f"Gagal memuat dataset: {e}")
        return pd.DataFrame(columns=ALL_COLS)

def save_dataset_crud(_df_save):
    """Simpan dataframe kembali ke CSV."""
    try:
        _df_save.to_csv(CSV_PATH, index=False)
        return True
    except Exception as e:
        st.error(f"Gagal menyimpan dataset: {e}")
        return False

def validate_numeric(val, col_name, min_val=1.0, max_val=5.0):
    """Validasi nilai numerik sesuai range."""
    try:
        v = float(val)
        if v < min_val or v > max_val:
            return False, f"Nilai {col_name} harus antara {min_val}–{max_val}"
        return True, v
    except Exception:
        return False, f"Nilai {col_name} harus berupa angka"
...
with st.sidebar:
    ...
    halaman = st.radio(
        "Navigasi",
        ["🏠 Beranda", "📊 Dataset", "🗄️ Kelola Dataset", "🔢 Hitung SPK", "📈 Visualisasi", "👥 Profil Kelompok"],
...
# ══════════════════════════════════════════════════════════════
# HALAMAN 3 — KELOLA DATASET (CRUD)
# ══════════════════════════════════════════════════════════════
elif halaman == "🗄️ Kelola Dataset":
    st.title("🗄️ Kelola Dataset Hotel")
    st.markdown("Tambah, edit, hapus, dan lihat data hotel langsung dari aplikasi. Perubahan tersimpan ke file CSV dan langsung digunakan sistem fuzzy.")
    st.markdown("---")

    # Load fresh dataset untuk CRUD
    df_crud = load_dataset_crud()

    # ── Metrik ringkasan ───────────────────────────────────────
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("📦 Total Data",    f"{len(df_crud):,}")
    m2.metric("🏨 Hotel Unik",    f"{df_crud['hotel_id'].nunique():,}")
    m3.metric("📋 Kolom",         f"{len(df_crud.columns)}")
    m4.metric("⭐ Avg Overall",   f"{df_crud['overall'].mean():.2f}")
    st.markdown("---")
    ...
    with tab_read:
        ...
        with col_s1:
            cari = st.text_input("🔍 Cari berdasarkan Hotel ID", placeholder="Contoh: 127781101")
        with col_s2:
            jumlah_tampil = st.selectbox("Tampilkan", [50, 100, 250, 500, "Semua"], index=1)

        df_tampil_read = df_crud.copy()
        if cari.strip():
            df_tampil_read = df_tampil_read[df_tampil_read["hotel_id"].str.contains(cari.strip(), case=False, na=False)]

        if jumlah_tampil != "Semua":
            df_tampil_read = df_tampil_read.head(int(jumlah_tampil))

        # Tampilkan kolom utama
        cols_read = [c for c in ALL_COLS if c in df_tampil_read.columns]
        df_show = df_tampil_read[cols_read].copy().reset_index(drop=True)
        df_show.index += 1

        st.dataframe(df_show, use_container_width=True, height=420)
        st.caption(f"Menampilkan {len(df_show):,} dari {len(df_crud):,} baris data.")

        # Download CSV
        csv_bytes = df_crud.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="⬇️ Unduh Dataset CSV",
            data=csv_bytes,
            file_name="data_baru.csv",
            mime="text/csv",
    ...
    # ══════════════════════
    with tab_create:
        st.markdown("### ➕ Tambah Data Hotel Baru")
        st.caption("Isi semua field di bawah. Nilai kriteria menggunakan skala 1–5.")

        with st.form("form_tambah_hotel", clear_on_submit=True):
            st.markdown("**Identitas Hotel**")
            f_id = st.text_input("Hotel ID *", placeholder="Contoh: 999001")

            st.markdown("**Nilai Kriteria (skala 1.0 – 5.0)**")
            fc1, fc2, fc3 = st.columns(3)
            with fc1:
                f_clean  = st.number_input("🧹 Cleanliness *", min_value=1.0, max_value=5.0, value=3.0, step=0.5)
                f_value  = st.number_input("💰 Value *",        min_value=1.0, max_value=5.0, value=3.0, step=0.5)
            with fc2:
                f_loc    = st.number_input("📍 Location *",     min_value=1.0, max_value=5.0, value=3.0, step=0.5)
                f_rooms  = st.number_input("🛏️ Rooms *",        min_value=1.0, max_value=5.0, value=3.0, step=0.5)
            with fc3:
                f_sleep  = st.number_input("😴 Sleep Quality *",min_value=1.0, max_value=5.0, value=3.0, step=0.5)
                f_overall= st.number_input("⭐ Overall *",       min_value=1.0, max_value=5.0, value=3.0, step=0.5)

            submitted_add = st.form_submit_button("✅ Simpan Data Hotel", type="primary", use_container_width=True)

        if submitted_add:
            errors = []
            hotel_id_baru = str(f_id).strip()

            # Validasi hotel_id
            if not hotel_id_baru:
                errors.append("Hotel ID tidak boleh kosong.")
            elif hotel_id_baru in df_crud["hotel_id"].values:
                errors.append(f"Hotel ID **{hotel_id_baru}** sudah ada di dataset. Gunakan ID yang berbeda.")

            # Validasi numerik sudah ditangani number_input, tapi double-check range
            for val, nama in [(f_clean,"Cleanliness"),(f_value,"Value"),(f_loc,"Location"),
                               (f_rooms,"Rooms"),(f_sleep,"Sleep Quality"),(f_overall,"Overall")]:
                ok, msg = validate_numeric(val, nama)
                if not ok:
                    errors.append(msg)
            ...
            if errors:
                ...
                # Buat baris baru — gunakan struktur kolom CSV asli
                new_row = {}
                for col in df_crud.columns:
                    if col == "hotel_id":      new_row[col] = hotel_id_baru
                    elif col == "cleanliness": new_row[col] = float(f_clean)
                    elif col == "value":       new_row[col] = float(f_value)
                    elif col == "location":    new_row[col] = float(f_loc)
                    elif col == "rooms":       new_row[col] = float(f_rooms)
                    elif col == "sleep_quality": new_row[col] = float(f_sleep)
                    elif col == "overall":     new_row[col] = float(f_overall)
                    else:                      new_row[col] = ""

                df_baru = pd.concat([df_crud, pd.DataFrame([new_row])], ignore_index=True)
                if save_dataset_crud(df_baru):
                    st.success(f"✅ Data hotel ID **{hotel_id_baru}** berhasil ditambahkan! Total data sekarang: **{len(df_baru):,}**")
                    st.cache_data.clear()
                    st.rerun()
    ...
    # ══════════════════════
    with tab_update:
        st.markdown("### ✏️ Edit Data Hotel")
        st.caption("Pilih Hotel ID, ubah nilai yang diinginkan, lalu simpan.")

        # Pilih hotel
        hotel_ids_list = sorted(df_crud["hotel_id"].unique().tolist())
        selected_id_edit = st.selectbox(
            "🔎 Pilih Hotel ID yang akan diedit",
            options=["— Pilih Hotel ID —"] + hotel_ids_list,
            key="select_edit"
        )

        if selected_id_edit and selected_id_edit != "— Pilih Hotel ID —":
            # Ambil data lama (baris pertama jika duplikat)
            row_lama = df_crud[df_crud["hotel_id"] == selected_id_edit].iloc[0]

            st.markdown(f"**Data saat ini untuk Hotel ID: `{selected_id_edit}`**")
            col_info = [c for c in ALL_COLS if c in df_crud.columns]
            st.dataframe(
                pd.DataFrame([row_lama[col_info]]).reset_index(drop=True),
                use_container_width=True
            )

            st.markdown("**Ubah nilai kriteria:**")
            with st.form("form_edit_hotel"):
                ec1, ec2, ec3 = st.columns(3)
                with ec1:
                    e_clean  = st.number_input("🧹 Cleanliness", min_value=1.0, max_value=5.0,
                                               value=float(row_lama.get("cleanliness", 3.0)), step=0.5)
                    e_value  = st.number_input("💰 Value",        min_value=1.0, max_value=5.0,
                                               value=float(row_lama.get("value", 3.0)), step=0.5)
                with ec2:
                    e_loc    = st.number_input("📍 Location",     min_value=1.0, max_value=5.0,
                                               value=float(row_lama.get("location", 3.0)), step=0.5)
                    e_rooms  = st.number_input("🛏️ Rooms",        min_value=1.0, max_value=5.0,
                                               value=float(row_lama.get("rooms", 3.0)), step=0.5)
                with ec3:
                    e_sleep  = st.number_input("😴 Sleep Quality",min_value=1.0, max_value=5.0,
                                               value=float(row_lama.get("sleep_quality", 3.0)), step=0.5)
                    e_overall= st.number_input("⭐ Overall",       min_value=1.0, max_value=5.0,
                                               value=float(row_lama.get("overall", 3.0)), step=0.5)

                submitted_edit = st.form_submit_button("💾 Simpan Perubahan", type="primary", use_container_width=True)

            if submitted_edit:
                # Update nilai di dataframe
                mask = df_crud["hotel_id"] == selected_id_edit
                df_crud.loc[mask, "cleanliness"]   = float(e_clean)
                df_crud.loc[mask, "value"]         = float(e_value)
                df_crud.loc[mask, "location"]      = float(e_loc)
                df_crud.loc[mask, "rooms"]         = float(e_rooms)
                df_crud.loc[mask, "sleep_quality"] = float(e_sleep)
                df_crud.loc[mask, "overall"]       = float(e_overall)

                if save_dataset_crud(df_crud):
                    st.success(f"✅ Data hotel ID **{selected_id_edit}** berhasil diperbarui!")
                    st.cache_data.clear()
                    st.rerun()
        else:
            st.info("⬆️ Pilih Hotel ID dari dropdown di atas untuk mulai mengedit.")

    # ══════════════════════
    # TAB 4 — DELETE
    # ══════════════════════
    with tab_delete:
        st.markdown("### 🗑️ Hapus Data Hotel")
        st.caption("Pilih Hotel ID yang akan dihapus. Konfirmasi diperlukan sebelum penghapusan permanen.")

        hotel_ids_del = sorted(df_crud["hotel_id"].unique().tolist())
        selected_id_del = st.selectbox(
            "🔎 Pilih Hotel ID yang akan dihapus",
            options=["— Pilih Hotel ID —"] + hotel_ids_del,
            key="select_delete"
        )

        if selected_id_del and selected_id_del != "— Pilih Hotel ID —":
            # Tampilkan preview data yang akan dihapus
            rows_to_del = df_crud[df_crud["hotel_id"] == selected_id_del]
            col_preview = [c for c in ALL_COLS if c in df_crud.columns]
            st.warning(f"⚠️ Data yang akan dihapus ({len(rows_to_del)} baris):")
            st.dataframe(rows_to_del[col_preview].reset_index(drop=True), use_container_width=True)

            st.markdown("---")
            st.markdown("**Konfirmasi Penghapusan**")
            konfirmasi = st.checkbox(
                f"Saya yakin ingin menghapus semua data dengan Hotel ID **{selected_id_del}**",
            ...
            if btn_hapus and konfirmasi:
                df_setelah = df_crud[df_crud["hotel_id"] != selected_id_del].reset_index(drop=True)
                jumlah_dihapus = len(df_crud) - len(df_setelah)
                if save_dataset_crud(df_setelah):
                    st.success(f"✅ **{jumlah_dihapus}** baris data dengan Hotel ID **{selected_id_del}** berhasil dihapus. Sisa data: **{len(df_setelah):,}**")
                    st.cache_data.clear()
                    st.rerun()
        else:
            st.info("⬆️ Pilih Hotel ID dari dropdown di atas untuk melanjutkan.")
